...
class ChatTestRunner {
  ...
  private messagesReceived: unknown[] = [];
    ...
    return new Promise((resolve, reject) => {
      const checkInterval = setInterval(() => {
        ...
        if (this.messagesReceived.length > initialMessageCount) {
          log(
            `  ✓ Received ${this.messagesReceived.length - initialMessageCount} new message(s)`,
          );
  ...
  async testUIRendering(): Promise<TestResult> {
    ...
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, TEST_CONFIG.verificationDelay),
      );

...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      try {
        ...
        if (storedMessages && storedMessages.length > 0) {
          ...
          const convertedMessages = storedMessages.map(
            (msg: CoreMessage, index: number) => ({
              id: `msg-${index}`,
              role: msg.role as "user" | "assistant",
              content:
                typeof msg.content === "string"
                  ? msg.content
                  : (msg.content as CoreMessagePart[]).find(
                      (p) => p.type === "text",
                    )?.text || "",
              timestamp: Date.now(),
              isStreaming: false,
  ...
  const deleteRecording = useCallback(async (recordingId: string) => {
    ...
    try {
      const result =
        await window.sidebarAPI.recorderDeleteRecording(recordingId);
      if (result.success) {
  ...
  useEffect(() => {
    ...
    const handleMessagesUpdated = (updatedMessages: CoreMessage[]): void => {
      console.log(
        "[CHAT] handleMessagesUpdated - received messages:",
    ...
    const handleShowRecordings = (recordingsList: Recording[]): void => {

...
export const RecorderControls: React.FC = () => {
  ...
  const handleAddManualStep = async (): Promise<void> => {
    try {
  ...
  const handleShowRecordings = async (): Promise<void> => {
    try {
  ...
  return (
    <div className="flex items-center gap-1">
      {!isRecording ? (
        <>
          <ToolBarButton Icon={Circle} onClick={handleStartRecording} />
          <ToolBarButton Icon={List} onClick={handleShowRecordings} />
        </>
      ) : (
        <>
          <ToolBarButton
          ...
          />
          <ToolBarButton Icon={Clock} onClick={handleAddManualStep} />
          <ToolBarButton
            Icon={Square}
